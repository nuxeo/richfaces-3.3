...
FileUploadEntry.clearControlTemplate = [ new E(
    'a',
    {
      'style' : '',
      'onclick' : function(context) {
        return 'var entry = FileUploadEntry.getComponent(this); entry.uploadObject.clear(entry); return false;';
      },
      'className' : function(context) {
        return 'rich-fileupload-anc '
            + Richfaces.evalMacro("className", context);
      },
      'href' : '#'
    }, [ new T(function(context) {
      return Richfaces.evalMacro("controlLink", context);
    }) ]) ];

FileUploadEntry.stopControlTemplate = [ new E(
    'a',
    {
      'style' : '',
      'onclick' : function(context) {
        return 'FileUploadEntry.getComponent(this).uploadObject.stop(); return false;';
      },
      'className' : function(context) {
        return 'rich-fileupload-anc '
            + Richfaces.evalMacro("className", context);
...
FileUploadEntry.cancelControlTemplate = [ new E(
    'a',
    {
      'style' : '',
      'onclick' : function(context) {
        return 'var entry = FileUploadEntry.getComponent(this); entry.uploadObject.clear(entry, true); return false;';
      },
      'className' : function(context) {
        return 'rich-fileupload-anc '
            + Richfaces.evalMacro("className", context);
...
FileUploadEntry.template = [ new E(
    'table',
    {
      'cellspacing' : '0',
      'cellpadding' : '0',
        ...
        [ new E(
            ...
            [
                new E(
                    ...
                    {
                      'className' : function(context) {
                        return 'rich-fileupload-font rich-fileupload-name rich-fileupload-table-td '
                            + Richfaces.evalMacro("className", context);
                      }
                    }, [
                        new E('div', {
                          'className' : 'rich-fileupload-name-padding',
                          'style' : function(context) {
                            return 'overflow : hidden; width:'
                                + Richfaces
                                    .evalMacro("fileEntryWidth", context);
                          }
                        }, [ new ET(function(context) {
                          return Richfaces.evalMacro("fileName", context)
                        }) ]),
                        new E('div', {}),
                        new E('div', {
                          'className' : 'rich-fileupload-name-padding',
                          ...
                          return Richfaces.evalMacro("label", context)
                        }) ]) ]), new E('td', {
                  'style' : 'vertical-align: center;',
                  'className' : 'rich-fileupload-table-td'
                }, [ new E('div', {
                  'className' : 'rich-fileupload-font rich-fileupload-del'
                }, [
                // FileUploadEntry.clearControlTemplate
                ]) ]), new E('td', {
                  'className' : 'rich-fileupload-table-td'
                }, [ new E('div', {
                  'className' : 'rich-fileupload-font rich-fileupload-scroll'
                }, [ new T('\u00A0') ]) ]) ]) ]) ]) ];
...
Object
    .extend(
        FileUploadEntry.prototype,
        {
          ...
          initialize : function(fileInput, uploadObject, size, type, creator,
            ...
            var content = FileUploadEntry.template.invoke('getContent', {
              fileName : fileName,
              fileEntryWidth : uploadObject.getFileEntryWidth(),
              className : this.uploadObject.classes.FILE_ENTRY.ENABLED
            }).join('');
          ...
          setupLabelUpdate : function() {
            this.updateLabel();
            this.labelUpdateInterval = setInterval(function() {
              this.updateLabel();
          ...
          updateLabel : function() {
            if (this.state != FileUploadEntry.UPLOAD_IN_PROGRESS) {
              ...
              if (p) {
                var content = this.uploadObject.labelMarkup.invoke(
                    'getContent', this.uploadObject.progressData.getContext(p))
                    .join('');
                  ...
                  .update(
                      ...
                      FileUploadEntry.stopControlTemplate
                          .invoke(
                              'getContent',
                              {
                                'controlLink' : FileUploadEntry.LABELS['entry_stop'],
                                'className' : this.uploadObject.classes.FILE_ENTRY_CONTROL.ENABLED
                              }).join(''));
                  ...
                  .update(
                      ...
                      FileUploadEntry.clearControlTemplate
                          .invoke(
                              'getContent',
                              {
                                'controlLink' : FileUploadEntry.LABELS['entry_clear'],
                                'className' : this.uploadObject.classes.FILE_ENTRY_CONTROL.ENABLED
                              }).join(''));
            } else {
              Element
                  .update(
                      ...
                      FileUploadEntry.cancelControlTemplate
                          .invoke(
                              'getContent',
                              {
                                'controlLink' : FileUploadEntry.LABELS['entry_cancel'],
                                'className' : this.uploadObject.classes.FILE_ENTRY_CONTROL.ENABLED
                              }).join(''));
            }
...
Object.extend(LoadWatcher.prototype, {
      ...
      if (!this.stopped) {
        ...
        try {
          if (!Prototype.Browser.Opera && !Prototype.Browser.WebKit
            ...
            loaded = /complete/
                .test(this.iframe.contentWindow.document.readyState);
          }
        } catch (e) {
          error = e;
        }
  ...
  onload : function() {
    ...
    var restr = iframeDocument
        .getElementById('_richfaces_file_upload_size_restricted');
    var forb = iframeDocument
        .getElementById('_richfaces_file_upload_forbidden');
...
Object
    .extend(
        FileUpload.prototype,
        {
          ...
          initialize : function(id, formId, actionUrl, stopScript,
              getFileSizeScript, progressBarId, classes, label, maxFiles,
              events, disabled, acceptedTypes, options, labels, parameters,
              sessionId) {
            this.id = id;
          ...
          createFrame : function() {
            ...
            var child = "<iframe name='" + this.id + "_iframe' id='" + this.id
                + "_iframe'"
                + (!Richfaces.browser.isOpera ? " src=\"javascript:''\"" : "")
                + "></iframe>";
            div.innerHTML = child;
          ...
          remove : function(entry) {
            entry.clear();
            if (this.isFlash)
              this._flashRemoveFile(this.entries.indexOf(entry));
          ...
          _selectEntryForUpload : function() {
            ...
            for ( var i = 0; i < l; i++) {
              ...
              if (entry.state == FileUploadEntry.READY
          ...
          cleanAllDisabled : function() {
            if (this.options['autoclear'])
              return true;
            var c = this
                .getFileEntriesSumByState(FileUploadEntry.UPLOAD_SUCCESS);
            return (c == 0);
          },
          ...
          disableCleanButton : function() {
            ...
            this._updateClassNames(d1, d2, this.classes.CLEAN,
                this.classes.CLEAN_CONTENT);

          },

          disableAddButton : function() {
            var disabled = ((this.getFileEntriesSumByState(
                FileUploadEntry.READY, FileUploadEntry.INITIALIZED,
                FileUploadEntry.UPLOAD_CANCELED)
                + this.uploadedCount + this
                .getFileEntriesSumByState(FileUploadEntry.UPLOAD_IN_PROGRESS)) >= this.maxFileBatchSize);
            ...
            this._updateClassNames(d1, d2, this.classes.ADD,
                this.classes.ADD_CONTENT);
          ...
          disableUploadButton : function() {
            ...
            this._updateClassNames(d1, d2, (this.runUpload) ? this.classes.STOP
                : this.classes.UPDATE,
                (this.runUpload) ? this.classes.STOP_CONTENT
          ...
          _updateClassNames : function(d1, d2, buttonClass, buttonContentClass) {
            if (this.disabled) {
              ...
              d1.onmouseover = function() {
                this.className = 'rich-fileupload-button-light rich-fileupload-font';
              };
              d1.onmouseout = function() {
                this.className = 'rich-fileupload-button rich-fileupload-font';
              };
              d1.onmousedown = function() {
                this.className = 'rich-fileupload-button-press rich-fileupload-font';
              };
          ...
          disable : function() {
            this.disabled = true;
            this.items.className = "rich-fileupload-list-overflow "
                + this.classes.UPLOAD_LIST.DISABLED;
            for ( var i = 0; i < this.entries.length; i++) {
              var entry = this.entries[i];
              entry.entryElement.rows[0].cells[0].className = "rich-fileupload-font rich-fileupload-name rich-fileupload-table-td "
                  + this.classes.FILE_ENTRY.DISABLED;
              entry.controlArea.firstChild.className = "rich-fileupload-anc "
          ...
          enable : function() {
            ...
            for ( var i = 0; i < this.entries.length; i++) {
              var entry = this.entries[i];
              entry.entryElement.rows[0].cells[0].className = "rich-fileupload-font rich-fileupload-name rich-fileupload-table-td "
                  + this.classes.FILE_ENTRY.ENABLED;
              entry.controlArea.firstChild.className = "rich-fileupload-anc "
          ...
          notifyStateChange : function(entry, oldState) {
            ...
            if (newState == FileUploadEntry.UPLOAD_SUCCESS
                || newState == FileUploadEntry.UPLOAD_SIZE_ERROR) {
          ...
          _getForm : function() {
            ...
            while (parentForm.tagName
                && parentForm.tagName.toLowerCase() != 'form') {
          ...
          submitForm : function(entry) {
            ...
            try {
              if (!formUpload) {
                ...
                Richfaces.writeAttribute(parentForm, "action", this.actionUrl
                    + (/\?/.test(this.actionUrl) ? '&_richfaces_upload_uid'
                        : '?_richfaces_upload_uid') + '='
                    + encodeURI(entry.uid) + "&" + this.id + "=" + this.id
                    + "&_richfaces_upload_file_indicator=true"
                    + "&AJAXREQUEST=" + this.progressBar.containerId);
              ...
              if (!parentForm.onsubmit || parentForm.onsubmit()) {
                if (!formUpload) {
                  this.watcher = new LoadWatcher(this.iframe,
                      function(newState) {
                        this.finishProgressBar();
                        this.setState(newState);
                        // call onFileUploadComplete event handler
                        if (this.uploadObject.events.onfileuploadcomplete) {
                          this.uploadObject.element.fire(
                              "rich:onfileuploadcomplete", {
                                state : newState
                              });
                        }
                      }.bind(entry), function(state) {
          ...
          beforeSubmit : function() {
            ...
            Richfaces.writeAttribute(f, "action", this.actionUrl
                + (/\?/.test(this.actionUrl) ? '&_richfaces_upload_uid'
          ...
          initFlashModule : function() {
            ...
            if (allowFlash == "auto" || allowFlash == "true") {
              ...
              div.innerHTML = '<a href="http'
                  + httpsSuffix
                  + '://www.adobe.com/go/getflashplayer"><img style="border:0px; margin:2px" src="http'
                  + httpsSuffix
              ...
              if (this.isFlash) {
                ...
                this.currentInput.parentNode.innerHTML = '<input type="text" style="cursor: pointer; right: 0px; top: 0px; font-size: 10em; position: absolute; padding: 0px" class="rich-fileupload-hidden" id="'
                    + this.id + ':file" name="' + this.id + ':file"></input>'
                this.currentInput = $(this.id + ":file");

                this.currentInput.style.display = "none";
                this.currentInput.onmousedown = (function() {
                  this.createFrame();
                }).bind(this);
          ...
          _flashSetComponent : function() {
            ...
            this.flashComponent = (document[flashId]) ? document[flashId]
                : (window[flashId] ? window[flashId] : $(flashId));
          ...
          _flashAdd : function(files) {
            ...
            for ( var i = this.entries.length; i < files.length; i++) {
              ...
              var newEntry = new FileUploadEntry(this.currentInput, this,
                  file.size, file.type, file.creator, file.creationDate,
          ...
          _flashGetActionUrl : function(url, entry) {
            var getParams = "_richfaces_upload_uid=" + encodeURI(entry.uid)
                + "&" + this.id + "=" + this.id
                + "&_richfaces_upload_file_indicator=true&_richfaces_size="
                + entry.size + "&_richfaces_send_http_error=true";
            if (/\?/.test(url)) {
              var i = url.indexOf("?");
          ...
          _flashOnComplete : function() {
            ...
            this._flashSetEntryState(this.uploadIndex,
                (this.upload_stopped ? FileUploadEntry.UPLOAD_CANCELED
                    : FileUploadEntry.UPLOAD_SUCCESS));
            this.upload_stopped = false;
          },

          _flashHTTPError : function(httpErrorNumber) {
            if (httpErrorNumber == 413)
              this._flashError(FileUploadEntry.UPLOAD_SIZE_ERROR);
            else
              this._flashError();
          },
          ...
          _flashError : function(error) {
            this.finishProgressBar();
            this._flashSetEntryState(this.uploadIndex,
                (error == undefined ? FileUploadEntry.UPLOAD_TRANSFER_ERROR
                    : error));
          },
          ...
          _flashSetEntryState : function(entryIndex, state) {
            ...
            if (entry) {
              ...
              if (this.events.onfileuploadcomplete) {
                this.element.fire("rich:onfileuploadcomplete", {
                  state : state
                });
              }
            }
